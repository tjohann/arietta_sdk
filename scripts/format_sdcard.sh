...
################################################################################
#
# Date/Beginn :    15.02.2017/12.07.2016
#
# Version     :    V0.01
#
...
{
    if [[ -b ${DEVNODE}1 ]]; then
	echo "sudo mkfs.vfat -F 32 -n KERNEL_ARIE ${DEVNODE}1"
	sudo mkfs.vfat -F 32 -n KERNEL_ARIE ${DEVNODE}1
	if [ $? -ne 0 ] ; then
    ...
    if [[ -b ${DEVNODE}2 ]]; then
	echo "sudo mkfs.ext4 -O ^has_journal -L ROOTFS_ARIE ${DEVNODE}2"
	sudo mkfs.ext4 -O ^has_journal -L ROOTFS_ARIE ${DEVNODE}2
	if [ $? -ne 0 ] ; then
    ...
    if [[ -b ${DEVNODE}3 ]]; then
	echo "sudo mkfs.ext4 -O ^has_journal -L HOME_ARIE ${DEVNODE}3"
	sudo mkfs.ext4 -O ^has_journal -L HOME_ARIE ${DEVNODE}3
	if [ $? -ne 0 ] ; then
